...
class Detection:
    def __init__(self):
        caffemodel = "antispoofing/Widerface-RetinaFace.caffemodel"
        deploy = "antispoofing/deploy.prototxt"
    ...
    def get_bbox(self, img):
        ...
        if img.shape[1] * img.shape[0] >= 192 * 192:
            img = cv2.resize(img, (int(192 * math.sqrt(aspect_ratio)), int(192 / math.sqrt(aspect_ratio))), interpolation=cv2.INTER_LINEAR)
...
class AntiSpoofPredict(Detection):
    ...
    def predict(self, img, model_path):
        ...
        self.model.eval()
        with torch.no_grad(): result = F.softmax(self.model.forward(img), dim = 1).cpu().numpy()
